...
int main() {
    ...
    Rational prod_r1_5 = r1 * 5;        // 正确。'='右边等价于r1.operator*(Rational(5))
    Rational prod_5_r1 = 5 * r1;        // 错误。'='右边等价于5.operator*(r1)。5是内置类型，不存在成员函数
                                        // 此时，编译器还会试图寻找非成员函数的operator*()，满足operator*(int, Rational)，但并未找到
    return 0;
}